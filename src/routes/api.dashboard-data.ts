...
const VACANCY_PATTERNS = {
  filled:    [/^1$/, /fill(ed)?/i, /close(d)?/i, /hired?/i, /placed?/i, /joined?/i, /onboard(ed|ing)?/i, /accepted/i, /offer.*accepted/i],
  onHold:    [/hold/i, /pause(d)?/i, /defer(red)?/i, /suspend(ed)?/i, /frozen/i, /postponed/i],
  inProcess: [/^0$/, /process/i, /progress/i, /open/i, /active/i, /ongoing/i, /live/i, /available/i, /recruiting/i],
}
...
function getQuarter(dateStr: string, quarter: string): string {
  ...
  if (!parsed) return 'Q1'
  return monthToQuarter(parsed.getMonth() + 1)
}
...
function computeDashboard(applicants: Record<string, string>[], vacancies: Record<string, string>[]) {
  ...
  const allKeys = Object.keys(apps[0] || {})
  ...
  for (const row of apps) {
    ...
    const quarter   = getQuarter(dateStr, row[quarterKey] || '')
    ...
    if (isDrop || isR1 || isR2 || isScreenReject || isOfferDrop) {
      ...
      if (reason && reason !== '-' && reason.toLowerCase() !== 'na' && reason.length > 2) {
        if (isOfferDrop) offerDropReasonMap[reason] = (offerDropReasonMap[reason] || 0) + 1
        else             rejectReasonMap[reason]     = (rejectReasonMap[reason] || 0) + 1
      }
    }

    if (isJoin) {
      joined++; shortlisted++; interviewed++; offered++
      sourceMap[source].joined++; buMap[bu].joined++
      posMap[position].joined++; recruiterMap[recruiter].joined++
      recruiterMap[recruiter].offers++; quarterMap[quarter].joined++
      const appDate  = parseDate(dateStr)
      const joinDate = parseDate(row[joinDateKey] || '')
      if (appDate && joinDate && joinDate > appDate) {
        const days = Math.round((joinDate.getTime() - appDate.getTime()) / 86400000)
        if (days > 0 && days < 365) {
          timeToFillDays.push(days)
          if (!timeToFillByBUMap[bu]) timeToFillByBUMap[bu] = []
          timeToFillByBUMap[bu].push(days)
        }
      }
    } else if (isOfferDrop) {
      offerDrops++; offered++; shortlisted++; interviewed++
      recruiterMap[recruiter].offers++; recruiterMap[recruiter].offerDrops++
    } else if (isOffer)        { offered++; shortlisted++; interviewed++; recruiterMap[recruiter].offers++ }
    else if (isDrop)           { candidateDrops++; if (isShort) shortlisted++; if (isInterview) interviewed++ }
    else if (isR1)             { r1Rejects++; shortlisted++; interviewed++ }
    else if (isR2)             { r2Rejects++; shortlisted++; interviewed++ }
    else if (isNoShow)         { noShows++; shortlisted++ }
    else if (isScreenReject)   { screenRejects++ }
  ...
  for (const row of vacs) {
    ...
    vacBUMap[bu].total++
    if      (matchStatus(status, VACANCY_PATTERNS.filled))    { filledVacancies++;    vacBUMap[bu].filled++ }
    else if (matchStatus(status, VACANCY_PATTERNS.onHold))    { onHoldVacancies++;    vacBUMap[bu].onHold++ }
    else                                                       { inProcessVacancies++; vacBUMap[bu].inProcess++ }
  }
  ...
  const fillRate            = totalVacancies > 0
    ? (filledVacancies > 0 ? (filledVacancies / totalVacancies) * 100 : (joined / totalVacancies) * 100)
    : 0
  ...
  return {
    kpis: { totalApplicants, joined, hiringRate, offersExtended, offerDropRate, offerAcceptanceRate, candidateDrops, screenRejects, totalVacancies, filledVacancies, onHoldVacancies, inProcessVacancies, fillRate, avgTimeToFill },
    funnel: { applied: totalApplicants, shortlisted, interviewed, offered, joined },
    sourceEfficiency: Object.entries(sourceMap).map(([source, d]) => ({ source, total: d.total, joined: d.joined, rate: d.total > 0 ? (d.joined / d.total) * 100 : 0 })).sort((a, b) => b.rate - a.rate),
    buPerformance:    Object.entries(buMap).map(([bu, d]) => ({ bu, total: d.total, joined: d.joined, rate: d.total > 0 ? (d.joined / d.total) * 100 : 0 })).sort((a, b) => b.total - a.total),
    topRejectionReasons: Object.entries(rejectReasonMap).map(([reason, count]) => ({ reason, count })).sort((a, b) => b.count - a.count).slice(0, 10),
    leakage: { candidateDrops, r1Rejects, r2Rejects, noShows, offerDrops, screenRejects },
    quarterlyTrend: ['Q1', 'Q2', 'Q3', 'Q4'].map(q => ({ quarter: q, applicants: quarterMap[q]?.applicants || 0, joined: quarterMap[q]?.joined || 0 })),
    recruiterPerformance: Object.entries(recruiterMap).filter(([n]) => n !== 'Unknown').map(([recruiter, d]) => ({ recruiter, applications: d.apps, offers: d.offers, joined: d.joined, convRate: d.apps > 0 ? (d.joined / d.apps) * 100 : 0, offerDropRate: d.offers > 0 ? (d.offerDrops / d.offers) * 100 : 0 })).sort((a, b) => b.joined - a.joined),
    topPositions: Object.entries(posMap).filter(([p]) => p !== 'Unknown').map(([position, d]) => ({ position, apps: d.apps, joined: d.joined })).sort((a, b) => b.apps - a.apps).slice(0, TOP_POSITIONS_LIMIT),
    vacancyByBU:  Object.entries(vacBUMap).filter(([bu]) => bu !== 'Unknown').map(([bu, d]) => ({ bu, ...d })).sort((a, b) => b.total - a.total),
    topOfferDropReasons: Object.entries(offerDropReasonMap).map(([reason, count]) => ({ reason, count })).sort((a, b) => b.count - a.count).slice(0, 5),
    timeToFillByBU:      Object.entries(timeToFillByBUMap).filter(([bu]) => bu !== 'Unknown').map(([bu, days]) => ({ bu, avgDays: Math.round(days.reduce((a, b) => a + b, 0) / days.length) })).sort((a, b) => b.avgDays - a.avgDays),
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const applicantsUrl = url.searchParams.get('applicants') || DEFAULT_APPLICANTS
        const vacanciesUrl  = url.searchParams.get('vacancies')  || DEFAULT_VACANCIES

        try {
          const [appsRes, vacsRes] = await Promise.all([fetch(applicantsUrl), fetch(vacanciesUrl)])
          if (!appsRes.ok || !vacsRes.ok) {
            return Response.json({ error: 'Failed to fetch CSV data' }, { status: 502, headers: CORS })
          }
          const [appsText, vacsText] = await Promise.all([appsRes.text(), vacsRes.text()])
          const data = computeDashboard(parseCSV(appsText), parseCSV(vacsText))
